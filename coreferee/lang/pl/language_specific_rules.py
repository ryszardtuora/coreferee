...
class LanguageSpecificRulesAnalyzer(RulesAnalyzer):
    ...
    def get_dependent_siblings(self, token:Token) -> list:
        ...
        def add_siblings_recursively(recursed_token:Token, visited_set:set) -> None:
            ...
            if token != recursed_token and \
                    token.pos_ in ('VERB', 'AUX') and self.is_potential_anaphor(token) and \
                    recursed_token.pos_ in ('VERB', 'AUX') and \
                    self.is_potential_anaphor(recursed_token):
            ...
            if (recursed_token.dep_ in self.dependent_sibling_deps or
                    self.has_morph(recursed_token, 'Case', 'Ins')) and \
                    recursed_token != token:
            ...
            for child in (child for child in recursed_token.children if recursed_token.pos_ in
                    ...
                    self.has_morph(child, 'Case', 'Ins') and len([1 for c in child.children if
                    c.dep_ == 'case' and c.lemma_ == 'z']) > 0 and child not in visited_set):
                child_siblings_set = add_siblings_recursively(child, visited_set)
                siblings_set |= child_siblings_set
            for child in (child for child in recursed_token.children if recursed_token.pos_ in
                    ('VERB', 'AUX') and self.is_potential_anaphor(recursed_token) and
                    child.pos_ in self.noun_pos and
                    self.has_morph(child, 'Case', 'Ins') and len([1 for c in child.children if
                    c.dep_ == 'case' and c.lemma_ == 'z' and c.i - 1 == recursed_token.i]) > 0 and
            ...
            if recursed_token.dep_ != self.root_dep:
                ...
                for child in (child for child in recursed_token.head.children if
                        ...
                        self.has_morph(child, 'Case', 'Ins') and len([1 for c in child.children if
                        c.dep_ == 'case' and c.lemma_ == 'z' and c.i - 1 == recursed_token.i]) > 0
    ...
    def is_potential_anaphoric_pair(self, referred:Mention, referring:Token, directly:bool) -> bool:
        ...
        def get_gender_number_info(token):
            ...
            if self.has_morph(token, 'Number', 'Sing'):
                if self.has_morph(token, 'Gender', 'Masc'):
                    masc = True
                    if token.tag_ == 'PPRON3' and not self.has_morph(token, 'Case', 'Nom'):
                        neut = True
            ...
            if self.has_morph(token, 'Number', 'Plur'):
                if self.has_morph(token, 'Gender', 'Masc') and \
                        self.has_morph(token, 'Animacy', 'Hum') and \
                        token.dep_ != 'nmod': # 'ich'
                    virile = True
                elif (self.has_morph(token, 'Gender', 'Masc') and
                        self.has_morph(token, 'Animacy', 'Nhum')) or \
                        (self.has_morph(token, 'Gender', 'Masc') and
                        self.has_morph(token, 'Animacy', 'Inan')) or \
                        self.has_morph(token, 'Gender', 'Fem') or \
                        self.has_morph(token, 'Gender', 'Neut'):
                    nonvirile= True
        ...
        def get_gender_number_info_for_single_token(token):
            ...
            if not self.is_reflexive_possessive_pronoun(token):
                masc, fem, neut, nonvirile, virile = get_gender_number_info(token)
                if not (masc or fem or neut or nonvirile or virile) and \
                        token.dep_.startswith('nsubj') \
                        and token.head.pos_ in ('VERB', 'AUX'):
                    masc, fem, neut, nonvirile, virile = get_gender_number_info(token.head)
        ...
        def are_coordinated_tokens_possibly_virile(tokens:list) -> int:
            ...
            for token in tokens:
                if self.has_morph(token, 'Gender', 'Masc') and \
                        self.has_morph(token, 'Animacy', 'Hum'):
                    return 2
                if self.has_morph(token, 'Gender', 'Masc') and \
                        self.has_morph(token, 'Animacy', 'Nhum'):
                    masc = True
                if self.has_morph(token, 'Gender', 'Masc') and \
                        self.has_morph(token, 'Animacy', 'Inan'):
                    masc = True
                if self.has_morph(token, 'Gender', 'Fem'):
                    fem = True
                if self.has_morph(token, 'Gender', 'Neut'):
            ...
            if (masc and fem) or (masc and neut) or (fem and neut):
                if tokens[0].dep_.startswith('nsubj') and tokens[0].head.pos_ in ('VERB', 'AUX'):
                    _, _, _, head_nonvirile, head_virile = get_gender_number_info(tokens[0].head)
    ...
    def is_reflexive_anaphor(self, token:Token) -> int:
        if token.tag_ == 'SIEBIE' or self.is_reflexive_possessive_pronoun(token):
            return 2
        if token.tag_ == 'PPRON3' and token.dep_ == 'nmod' and \
                self.has_morph(token, 'PrepCase', 'Npr'): # e.g. 'jego, jej, ich'
            return 1
        return 0
