...
def debug_structures(doc:Doc):
    for token in doc:
        print(token.i, token.text, token.lemma_, token.pos_, token.tag_,
            token.dep_, token.ent_type_, token.head.i, list(token.children))
    ...
    with lock:
        if language not in language_to_nlps:
            ...
            for config_entry in config:
                model_set.add('_'.join((language, config[config_entry]['model'])))
            nlps = []
