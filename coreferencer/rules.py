    ...
    def get_rules_analyzer(nlp):
        ...
        def read_in_data_files(directory:str, rules_analyzer):
            for data_filename in (filename for filename in
                    pkg_resources.resource_listdir(__name__, sep.join((
                    'lang', directory, 'data'))) if filename.endswith('.dat')):
                full_data_filename = pkg_resources.resource_filename(__name__, sep.join((
                    'lang', directory, 'data', data_filename)))
                with open(full_data_filename, "r", encoding="utf-8") as file:
                    setattr(rules_analyzer, data_filename[:-4],
                        [v.strip() for v in file.read().splitlines() if
                        len(v.strip()) > 1 and not v.strip().startswith('#')])
        ...
        with lock:
            if language not in language_to_rules:
                ...
                rules_analyzer.exclusively_male_names = [name for name in rules_analyzer.male_names
                    if name not in rules_analyzer.female_names]
                rules_analyzer.exclusively_female_names = [name for name in
                    rules_analyzer.female_names if name not in rules_analyzer.male_names]
...
class RulesAnalyzer(ABC):
    ...
    def initialize(self, doc:Doc) -> None:
        ...
        for token in doc:
            ...
            for sibling in (sibling for sibling in siblings_list if sibling.i != token.i):
        ...
        for token in doc:
            ...
            if self.is_potential_anaphor(token):
                ...
                this_sentence_number = doc._.coref_chains.temp_sent_starts.index(
                    this_sentence_start_index)
                start_sentence_number = 0
                if this_sentence_number > self.maximum_anaphora_sentence_referential_distance:
                    start_sentence_number = this_sentence_number - \
                        self.maximum_anaphora_sentence_referential_distance
                for preceding_token in (t for t in
                        doc[doc._.coref_chains.temp_sent_starts[start_sentence_number]: token.i] if
                        (self.is_potential_anaphor(t) or
                        self.is_independent_noun(t))):
                    simple_referred = Mention(preceding_token, False)
                    if self.language_independent_is_potential_anaphoric_pair(
                            simple_referred, token) > 0 and not \
                            self.is_potential_reflexive_pair(Mention(token, False),
                            doc[simple_referred.root_index]):
                        potential_referreds.append(simple_referred)
                    if len(preceding_token._.coref_chains.temp_dependent_siblings) > 0:
                        complex_referred = Mention(preceding_token, True)
                        if self.language_independent_is_potential_anaphoric_pair(
                                complex_referred, token) > 0:
                            potential_referreds.append(complex_referred)
                if this_sentence_number + 1 == len(doc._.coref_chains.temp_sent_starts):
                    succeeding_tokens = doc[token.i + 1:]
                else:
                    succeeding_tokens = doc[token.i + 1:
                        doc._.coref_chains.temp_sent_starts[this_sentence_number + 1]]
                for succeeding_token in (t for t in succeeding_tokens if
                        (self.is_potential_anaphor(t) or
                        self.is_independent_noun(t))):
                    simple_referred = Mention(succeeding_token, False)
                    if self.language_independent_is_potential_anaphoric_pair(
                            simple_referred, token) > 0 and \
                            (self.is_potential_cataphoric_pair(simple_referred, token) or
                            self.is_potential_reflexive_pair(simple_referred, token)):
                        potential_referreds.append(simple_referred)
                    if len(succeeding_token._.coref_chains.temp_dependent_siblings) > 0:
                        complex_referred = Mention(succeeding_token, True)
                        if self.language_independent_is_potential_anaphoric_pair(
                                complex_referred, token) > 0 and \
                                self.is_potential_cataphoric_pair(simple_referred, token):
                            potential_referreds.append(complex_referred)
    ...
    def language_independent_is_potential_anaphoric_pair(
            self, referred:Mention, referring:Token) -> bool:
        ...
        if result == 2 and not self.is_potential_anaphor(referred_root):
            ...
            while True:
                referring_or_governor_subtree = list(referring_or_governor.subtree)
                if referred_root in referring_or_governor_subtree:
                    break
                for referring_sub_token in referring_or_governor_subtree:
                    for referred_token in (doc[i] for i in referred.token_indexes):
        ...
        if result == 2 and referred_root._.coref_chains.temp_quote_array != \
    ...
    def has_list_member_in_propn_subtree(self, token:Token, word_list:list) -> bool:
    ...
    def get_propn_subtree(self, token:Token) -> list:
        ...
        if token.dep_ != self.root_dep and token.dep_ not in self.dependent_sibling_deps and \
                token.head.pos_ in self.propn_pos:
            return []
        subtree = list(token.subtree)
        before_start_index = -1
        after_end_index = sys.maxsize
        for subtoken in subtree:
            if subtoken.pos_ not in self.propn_pos and subtoken.i < token.i and \
                    before_start_index < subtoken.i:
                before_start_index = subtoken.i
            elif subtoken.pos_ not in self.propn_pos and subtoken.i > token.i and \
    ...
    def is_involved_in_non_or_conjunction(token:Token) -> bool:
        ...
        if token._.coref_chains.temp_governing_sibling is not None:
            return not \
                token._.coref_chains.temp_governing_sibling._.coref_chains.temp_has_or_coordination
        return False
    ...
    def is_token_in_one_of_phrases(token:Token, phrases:list) -> bool:
        ...
        for phrase in phrases:
            ...
            if phrase.lower() == ' '.join([t.text.lower() for t in doc[start_index: end_index]]):
                return True
        return False
    ...
    def is_potential_cataphoric_pair(self, referred:Mention, referring:Token) -> bool:
        ...
        for referring_verb_ancestor in (t for t in
                referring_inclusive_ancestors if t.pos_ in self.clause_root_pos and t not in
                referred_verb_ancestors):
