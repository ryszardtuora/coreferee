        ...
        def resolve_recursively(token:Token) -> list:
            ...
            for chain in token._.coref_chains.chains:
                for mention in (mention for mention in
                        chain.mentions if len(mention.token_indexes) > 1 and
                        token.i not in mention.token_indexes):
                    # Mention contains multiple tokens, some of which may be anaphors and
                    # belong to further chains.
                    for contained_token in (token.doc[index] for index in mention.token_indexes
                            if index != token.i):
                        tokens_to_return.update(
                            resolve_recursively(contained_token))
                    return tokens_to_return
            for chain in token._.coref_chains.chains:
                if len([mention for mention in chain.mentions if len(mention.token_indexes) > 1
                        and token.i in mention.token_indexes]) > 0:
                    ...
                    continue
                return {token.doc[chain.mentions[chain.most_specific_mention_index].root_index]}
            return {token}
...
class Chain:
    ...
    def __str__(self):
        return ': '.join((str(self.index), ', '.join(str(mention) for mention in self.mentions)))
    ...
    def __init__(self, root:Token=None, include_dependent_siblings:bool=False):
        if root is not None: # root==None during deserialization, never otherwise
            ...
            if include_dependent_siblings:
                self.token_indexes.extend([t.i for t in
                root._.coref_chains.temp_dependent_siblings])
            if len(self.token_indexes) > 1:
                self.pretty_representation = ''.join(('[',
                '; '.join(''.join((doc[token_index].text, '(',
                str(token_index), ')')) for token_index in self.token_indexes), ']'))
            else:
                self.pretty_representation = ''.join((doc[self.root_index].text, '(',
                str(self.root_index), ')'))
...
class FeatureTable:
    ...
    def __init__(self, *, tags:list, morphs:list, ent_types:list, lefthand_deps_to_children:list,
            righthand_deps_to_children:list, lefthand_deps_to_parents:list,
            righthand_deps_to_parents:list, parent_tags:list, parent_morphs:list,
            parent_lefthand_deps_to_children:list, parent_righthand_deps_to_children:list):
